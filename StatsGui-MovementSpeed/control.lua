...
local function getEnergyInInventory(player)
	...
	if (current_fuel) then
		...
		for _, fuel in pairs(fuels) do
			local fuel_name = fuel.fuel_name
			if prototypes.item[fuel_name] then
				count = main_inventory.get_item_count(fuel_name)
				if count > 0 then
					energy_in_inventory = energy_in_inventory + count * prototypes.item[fuel_name].fuel_value
				end
			end
		end
...
local function sensor(player)
	...
	if (player.vehicle) then
		...
		if (player.mod_settings["statsgui-ms-max-speed-vehicle"].value) then
			...
			if (max_speed ~= 0 and max_speed ~= nil) then
				gui_value = string.format("%s/%03.0f km/h", current_speed, max_speed)
			else
	...
	-- player or jet not in map
	-- player.render_mode == defines.render_mode.game for some reasom also pass for map zoom in
	-- use controller_type as workaround
	elseif (player.controller_type == defines.controllers.character) then
		-- jet
		if player.mod_settings["statsgui-ms-jet-fuel"] and script.active_mods["jetpack"] and remote.call("jetpack", "is_jetpacking", player) then
			gui_title = {"statsgui-ms.title-jet"}
			local energy_in_inventory = getEnergyInInventory(player)
		...
		-- player
			...
			if (player.mod_settings["statsgui-ms-max-speed-player"].value) then
				...
				if (player.character_running_speed ~= 0) then
					gui_value = string.format("%s/%03.0f km/h (%+.0f%%)", current_speed, max_speed, relative_max_speed)
				else
					gui_value = string.format("%s km/h", current_speed)
				end
			else
